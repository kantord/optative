...
fn throw_js<'js>(ctx: &Ctx<'js>, msg: &str) -> rquickjs::Error {
    let escaped = msg
        ...
        .replace('\n', "\\n");
    let err_val = ctx
        .eval::<Value<'js>, _>(format!(r#"new Error("{escaped}")"#))
...
const MANAGED_FILE_PROPS_JS: &str = concat!(
    ...
    "key:f=>f.path,",
    "value:f=>f.hash,",
    "reconciler:globalThis.__esto_optative_set({observe:()=>JSON.parse(globalThis.__esto_glob(D+'/**/*'))",
...
fn fs_file_fn<'js>(ctx: Ctx<'js>, props: Object<'js>) -> rquickjs::Result<Value<'js>> {
    ...
    let results: Vec<Value<'js>> = files
        .iter()
        .map(|file| {
...
pub fn register_fs_file(ctx: &Ctx<'_>) -> rquickjs::Result<()> {
    ctx.globals()
        .set("__esto_fs_File", Function::new(ctx.clone(), fs_file_fn)?)?;
    Ok(())
}
...
pub fn register_fs_folder(ctx: &Ctx<'_>) -> rquickjs::Result<()> {
    ctx.globals().set(
        "__esto_fs_Folder",
        Function::new(ctx.clone(), fs_folder_fn)?,
    )?;
    Ok(())
}
...
pub fn register_fs_git_repo(ctx: &Ctx<'_>) -> rquickjs::Result<()> {
    ctx.globals().set(
        "__esto_fs_GitRepo",
        Function::new(ctx.clone(), fs_git_repo_fn)?,
    )?;
    Ok(())
}
...
fn scope_supervise<'js>(
    ...
    let scope_rels: Vec<String> = scope_abs
        .iter()
    ...
    let mut claim_map: std::collections::HashMap<String, ClaimEntry<'js>> =
    ...
    for claim in &claims {
        ...
        let matched: Vec<String> = match kind.as_str() {
            ...
            _ => {
                ...
                if scope_rels.contains(&n) || content.is_some() {
                    vec![n]
                } else {
                    vec![]
                }
        ...
        for rel in matched {
            let entry = claim_map.entry(rel.clone()).or_insert(ClaimEntry {
                content: None,
                bodies: vec![],
            });
            ...
            if let Some(ref f) = body_fn {
                entry.bodies.push(f.clone());
            }
        }
    }

    // Circuit breaker: too many pruned
    let prune_count = scope_rels
        .iter()
        .filter(|r| !claim_map.contains_key(*r))
        .count();
...
fn folder_enumerate<'js>(
    ...
    let results: Vec<Value<'js>> = dirs
        .iter()
        .map(|abs_dir| {
            let rel = root
                .map(|r| abs_dir[r.len() + 1..].to_string())
...
pub(super) fn fs_claim_file_fn<'js>(
    ctx: Ctx<'js>,
    props: Object<'js>,
) -> rquickjs::Result<Value<'js>> {
    let name_str = props
    ...
    let content: Value<'js> = if content_val.is_undefined() {
...
pub(super) fn fs_file_enumerate_fn<'js>(
    ctx: Ctx<'js>,
    root_dir: Value<'js>,
    pattern: String,
    render: Function<'js>,
    ...
    let rels: Vec<String> = root_opt
        .as_deref()
        .map(|r| paths.iter().map(|p| p[r.len() + 1..].to_string()).collect())
        .unwrap_or(paths);
    let results: Vec<Value<'js>> = rels
        .iter()
        .map(|file| {
            let arg = Object::new(ctx.clone())?;
            arg.set("file", file.as_str())?;
...
pub(super) fn fs_folder_enumerate_fn<'js>(
    ctx: Ctx<'js>,
    root_dir: Value<'js>,
    pattern: String,
    render: Function<'js>,
...
pub(super) fn fs_scope_supervise_fn<'js>(
    ctx: Ctx<'js>,
    parent_dir: String,
    name: String,
    render: Function<'js>,
